...
# 3 intervals of 5 mins = 15 minutes rolling window
WINDOW_MULTIPLIER = 3
...
def replay_pcap_with_timing(pcap_file):
    ...
    for idx in range(0, max_bucket_index + 1):
        window_packets = []
        
        for j in range(max(0, idx - WINDOW_MULTIPLIER + 1), idx + 1):
            if j in interval_buckets:
                window_packets.extend(interval_buckets[j])
        ...
        subprocess.run(["sudo", "/usr/bin/python3", "/home/kali/Detection_Testing/LSTM/crowdingRegressor.py"])
